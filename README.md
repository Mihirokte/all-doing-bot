...
3. **Backend on EC2** — Use either:
   - **GitHub Actions** (`.github/workflows/deploy-ec2.yml`): add secrets **`EC2_HOST`**, **`EC2_USER`**, **`EC2_SSH_KEY`** (see [docs/deployment/github-actions-ec2-autodeploy.txt](docs/deployment/github-actions-ec2-autodeploy.txt)). **Fast path:** from repo root run `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/sync-github-actions-ec2.ps1` after `gh auth login`. Optional variable **`EC2_AUTO_DEPLOY`** = `true` deploys on every push to `main`; otherwise use **Actions → Deploy backend (EC2) → Run workflow**.
